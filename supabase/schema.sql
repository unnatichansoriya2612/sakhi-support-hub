-- =====================================================================
-- Sakhi (सखी) — full database schema
-- Women-to-women, time-based care platform
...
-- Users can never escalate their own profile role.
...
-- Double-booking protection: a Care Partner can never hold two overlapping
-- live bookings.
do $$ begin
...
-- New auth user -> profile + role. Admin can NEVER be self-assigned.
...
begin
  ...
  if final_role = 'care_partner' then
    ...
    values (new.id, (select default_hourly_rate from public.platform_settings where id = 1))
...
-- Notify both sides whenever a booking is created or changes status.
...
-- Only approved Care Partners can receive bookings; totals must be honest.
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    new.duration_hours := round(extract(epoch from (new.end_time - new.start_time)) / 3600.0, 2);
...
-- user_roles: read only; changes are service_role / SQL only
...
-- Care Partners may not approve or verify themselves.
